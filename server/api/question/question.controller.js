    ...
    findLatestQuestionId(function(question) {
      ...
      findQuestionSet(req.body.questionSet).then(function(questionSet) {
        if (questionSet.isDefault) {
          return res
            .status(404)
            .send("Cannot create question in a default questionSet");
        } else {
    ...
    Question.findById(req.params.id, function(err, question) {
      ...
      findQuestionSet(question.questionSet).then(function(questionSet) {
        if (questionSet.isDefault) {
          return res
            .status(404)
            .send("Cannot update question of default questionSet");
        } else {
    ...
    Question.findById(req.params.id, function(err, question) {
      ...
      findQuestionSet(question.questionSet).then(function(questionSet) {
        if (questionSet.isDefault) {
          return res
            .status(404)
            .send("Cannot delete question of default questionSet");
        } else {
          Answer.find({
  ...
  exports.importQuestions = function(req, res) {
    QuestionSet.findOne({
      ...
      if (!questionSet.isDefault) {
          ...
          if (questions.length >= 1) {
            Question.collection.insert(questions, {}, function(err, result) {
              ...
              for (var i = 0; i < questionIds.length; i++) {
                updateQuestionInQuestionSetList(
                  req.params.questionSet,
                  questionIds[i]
                );
              }
        ...
        return res
          .status(500)
          .json("Cannot import questions on a default question set");
      }
    });
  };
